...
- **Sharding Strategy:** For the `Order` and `User` domains, once the data outgrows a single database node, we will shard the data.
  - _Tenant-based/User-ID based sharding:_ Orders will be sharded by `user_id` so that all orders for a specific user reside on the same database shard. A consistent hashing algorithm (or an orchestrator like Citus) will route queries to the correct shard.
  - _Hot Data vs Cold Data:_ Historical orders will be archived to a data warehouse or cheaper storage, keeping the active PostgreSQL shards small and fast.
